...
if __name__ == '__main__':
    ...
    hydrophobics_kd = [ i for i in 'IVLFCMA' ]
    positives = [ i for i in 'KRH' ]
    negatives = [ i for i in 'DE' ]
    others = [i for i in 'NQGPSTWY']
    print(others)
#    others = set([ i for i in 'KRHDE' ]) # testing 0716
    alls = hydrophobics_kd+positives+negatives+others
    print(alls)
...
#    for subsetno,subset in enumerate([hydrophobics_kd, positives, negatives, others]):
    for subsetno,subset in enumerate([alls]):
#        fig7c,ax7c = plt.subplots(1,1, figsize=(4.5,9/16*4.5))
        fig7c,ax7c = plt.subplots(1,1, figsize=(10,9/16*4.5))
        ...
        for filepath in data:
            ...
            for idx,amino in enumerate(alls):
                if amino in subset:
    ...
    #        patchlist.append(patchname)
        ...
        #                axexp2.plot(amino_acid_list,mean_type_proportion,linestyle='None')
            ax7c.errorbar(reduced_list, mean_type_proportion, yerr=2*1.96*mean_type_stderr,linestyle='None',marker='o',label=f'{sysname} {res}',elinewidth=2,markersize=5,)
#            ax7c.set_axis_off()

#        if subsetno==0: ax7c.legend()
#        if subsetno in [0]: ax7c.legend(loc='upper right', borderpad=0.2, labelspacing=0.2, frameon=True, borderaxespad=0.1, handletextpad=0 ,ncol=2,bbox_to_anchor=(0.6, 1.000), fancybox=True, shadow=True)
...
#        ax7c.set_xlabel(f'Amino Acid')
        ax7c.set_ylabel(f'Surface Proportion')                        
        plt.grid()
        plt.tight_layout()       
        fig7c.savefig(f'images/fig8-{subsetno}-testing0920.png', dpi=300)
